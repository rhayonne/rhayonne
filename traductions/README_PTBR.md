<div id="contein-all">
  <div id="topo">
    <p align="right">
      🌎 Escolha o idioma:
      <a href="../README.md">🇺🇸 English</a>
      |
      <a href="README_PTBR.md">🇧🇷 Português</a>
      |
      <a href="README_FR.md">🇫🇷 Français</a>
    </p>
    <h1 align="center">Olá 👋, eu sou Rhayonne Lopes</h1>
    <h3 align="center">Apaixonado por Tecnologia da Informação</h3>
    <p>
      Olá, tudo bem?
      <br />
      Meu nome é Rhayonne e você está visualizando meu perfil no GitHub, que inclui uma visão geral do que utilizo no meu dia a dia. Sou apaixonado por TI e código aberto, e busco sempre estudar e
      entender como tudo funciona. Devido a essa paixão, adquiri amplo conhecimento em TI: Firewalls, redes, Windows Server, Active Directory, bem como arquitetura, design e desenvolvimento de
      software web.
      <br />
      Possuo mestrado em Gestão e Arquitetura de Sistemas de Informação pelo CESI em Caen, Normandia (Bac +5). Estou sempre em busca de novos conhecimentos. Aqui no meu perfil do GitHub, você pode ver
      os projetos pessoais que desenvolvo para fins de aprendizado e também revisar conceitos que aprendi no passado. Se quiser conversar, envie-me uma mensagem privada no LinkedIn ou um e-mail;
      deixarei os links abaixo.
      <br />
      Se desejar me convidar para trabalhar em um projeto, seja pessoal ou profissional, entre em contato também. Terei prazer em ajudar.
    </p>
  </div>
  <br />
  <div id="professional-summary">
    <h2 align="left">💼 Resumo Profissional</h2>
    <h4>Desenvolvedor Full-stack - Clariane, Paris (Out 2022 – Out 2025)</h4>
    <ul>
      <li>Responsável pelo design, desenvolvimento e implantação (CI/CD) de aplicações utilizando JavaScript/Node.js e diversos frameworks como NestJS, React, Vue.js, ExpressJS e JavaScript puro.</li>
      <li>Gerenciamento de estado e busca de dados usando ferramentas avançadas como React Query, React Context, Pinia e Zustand.</li>
      <li>Liderei a arquitetura e desenvolvimento de um sistema de RH para gestão de contratos de estágio, incluindo documentação completa e implementação.</li>
      <li>
        Mantive contato direto com usuários finais e diretores de negócios para levantamento de requisitos de novas funcionalidades, melhorias e alinhamentos técnicos por meio de reuniões e webinars.
      </li>
      <li>Organizei e planejei a base de código de todas as aplicações sob minha responsabilidade, cobrindo tanto novos desenvolvimentos quanto manutenção preventiva/corretiva.</li>
      <li>Utilização de ClickUp, Jira e Figma para controle de tickets, gestão de projetos e alinhamento de UX/UI.</li>
      <li>Trabalhei com Palantir Foundry para construção de pipelines ETL e processamento de dados em larga escala (Big Data).</li>
      <li>Implementação de soluções de banco de dados usando MongoDB, SQL e PostgreSQL.</li>
      <li>Expertise no ecossistema Microsoft Azure, incluindo Azure Portal, Azure DevOps e gestão de identidades e controle de acesso via Azure IAM.</li>
      <li>Gerenciamento de controle de versão usando Docker e Azure Container Registry, estabelecendo o processo completo de CI/CD do desenvolvimento à entrega final ao cliente.</li>
      <li>Melhoria dos padrões de desenvolvimento e qualidade de código configurando SonarQube Community Server e implementando testes unitários abrangentes.</li>
    </ul>
    <h4>Gerente de Projetos de Integração - ERP - PcHelp Sistemas, Brasil (Abr 2014 – Jul 2017)</h4>
    <ul>
      <li>Gestão completa de projetos de integração de ERP, desde o levantamento de requisitos do cliente até a implementação da solução.</li>
      <li>Direção da instalação, configuração e integração de um ERP contábil e de gestão, incluindo migração e otimização de bancos de dados SQL.</li>
      <li>Design e desenvolvimento de relatórios SQL personalizados, e responsabilidade pela análise e resolução de anomalias, bem como testes e documentação.</li>
      <li>Treinamento de usuários em diferentes módulos do ERP.</li>
      <li>Suporte e implementação de módulos contábeis e fiscais.</li>
    </ul>
  </div>
  <br />
  <hr />
  <br />
  <div id="Education">
    <h2 align="left">🎓 Educação & Certificações</h2>
    <ul>
      <li>
        <strong>Manager en Architecture et Applications Logicielles (Bac+5)</strong>
        <br />
        Escola: CESI Caen
        <br />
        Período: 2023 - 2025
      </li>
      <br />
      <li>
        <strong>Concepteur Développeur d'Applications (Bac+3)</strong>
        <br />
        Escola: CESI Caen
        <br />
        Período: 2022 - 2023
      </li>
      <br />
      <li>
        <strong>Analista de Cybersecurity</strong>
        <br />
        Escola: PopSchool Caen
        <br />
        Período: 2022
      </li>
      <br />
      <li>
        <strong>Análise e Desenvolvimento de Sistemas (Bac+2)</strong>
        <br />
        Escola: Faculdade Cidade Verde, Brasil
        <br />
        Período: 2014 - 2016
      </li>
    </ul>
  </div>
  <br />
  <hr />
  <div id="networks">
    <h3 align="left">🤝 Conecte-se comigo</h3>
    <table align="left">
      <tr>
        <td align="center">
          <a href="https://linkedin.com/in/rhayonne-lopes" target="blank">
            <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" height="40" width="40" />
            <br />
            <sub>Linkedin</sub>
            <br />
          </a>
        </td>
        <td align="center">
          <a href="https://medium.com/@rhayonne" target="blank">
            <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/medium.svg" height="40" width="40" />
            <br />
            <sub>Medium</sub>
            <br />
          </a>
        </td>
      </tr>
    </table>
  </div>
  <br />
  <br />
  <br />
  <br />
  <hr />
  <br />
  <div id="stacks-tools">
    <h2 align="center">🛠️ Tech Stack & Ferramentas</h2>
    <div id="frontend-mobile">
      <h3 align="center">💻 Desenvolvimento Frontend & Mobile</h3>
      <table align="center">
        <tr>
          <td align="center">
            <a href="https://angular.io">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/angularjs/angularjs-original.svg" width="45" height="45" />
              <br />
              <sub>Angular</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://reactjs.org/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" width="45" height="45" />
              <br />
              <sub>React</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://reactnative.dev/">
              <img src="https://reactnative.dev/img/header_logo.svg" width="45" height="45" />
              <br />
              <sub>React Native</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://vuejs.org/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vuejs/vuejs-original.svg" width="45" height="45" />
              <br />
              <sub>Vue.js</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://nuxtjs.org/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nuxtjs/nuxtjs-original.svg" width="45" height="45" />
              <br />
              <sub>NuxtJS</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://nextjs.org/">
              <img src="https://cdn.worldvectorlogo.com/logos/nextjs-2.svg" width="45" height="45" />
              <br />
              <sub>Next.js</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.w3.org/html/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" width="45" height="45" />
              <br />
              <sub>HTML5</sub>
            </a>
          </td>
        </tr>
      </table>
    </div>
    <div id="uiFrameStyle">
      <h3 align="center">🎨 UI Frameworks & Estilização</h3>
      <table align="center">
        <tr>
          <td align="center">
            <a href="https://mui.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/materialui/materialui-original.svg" width="45" height="45" />
              <br />
              <sub>MUI</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://chakra-ui.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/chakraui/chakraui-original.svg" width="45" height="45" />
              <br />
              <sub>ChakraUI</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://tailwindcss.com/">
              <img src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg" width="45" height="45" />
              <br />
              <sub>Tailwind</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://sass-lang.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/sass/sass-original.svg" width="45" height="45" />
              <br />
              <sub>Sass</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://getbootstrap.com">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/bootstrap/bootstrap-plain.svg" width="45" height="45" />
              <br />
              <sub>Bootstrap</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.w3schools.com/css/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" width="45" height="45" />
              <br />
              <sub>CSS3</sub>
            </a>
          </td>
        </tr>
      </table>
    </div>
    <div id="backormlang">
      <h3 align="center">⚙️ Backend, ORM & Linguagens</h3>
      <table align="center">
        <tr>
          <td align="center">
            <a href="https://nestjs.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nestjs/nestjs-original.svg" width="45" height="45" />
              <br />
              <sub>NestJS</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://nodejs.org">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original.svg" width="45" height="45" />
              <br />
              <sub>Node.js</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://mongoosejs.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongoose/mongoose-original.svg" width="45" height="45" />
              <br />
              <sub>Mongoose</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://sequelize.org/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/sequelize/sequelize-original.svg" width="45" height="45" />
              <br />
              <sub>Sequelize</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://expressjs.com">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original.svg" width="45" height="45" />
              <br />
              <sub>Express</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.typescriptlang.org/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" width="45" height="45" />
              <br />
              <sub>TS</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.java.com">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="45" height="45" />
              <br />
              <sub>Java</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.python.org">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" width="45" height="45" />
              <br />
              <sub>Python</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.gnu.org/software/bash/">
              <img src="https://www.vectorlogo.zone/logos/gnu_bash/gnu_bash-icon.svg" width="45" height="45" />
              <br />
              <sub>Bash</sub>
            </a>
          </td>
        </tr>
      </table>
    </div>
    <div id="cloudDevOps">
      <h3 align="center">☁️ Cloud, DevOps & Qualidade</h3>
      <table align="center">
        <tr>
          <td align="center">
            <a href="https://azure.microsoft.com/">
              <img src="https://www.vectorlogo.zone/logos/microsoft_azure/microsoft_azure-icon.svg" width="45" height="45" />
              <br />
              <sub>Azure</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://azure.microsoft.com/en-us/products/devops/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/azuredevops/azuredevops-original.svg" width="45" height="45" />
              <br />
              <sub>DevOps</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://cloud.google.com">
              <img src="https://www.vectorlogo.zone/logos/google_cloud/google_cloud-icon.svg" width="45" height="45" />
              <br />
              <sub>GCP</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://firebase.google.com/">
              <img src="https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg" width="45" height="45" />
              <br />
              <sub>Firebase</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.docker.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" width="45" height="45" />
              <br />
              <sub>Docker</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://kubernetes.io">
              <img src="https://www.vectorlogo.zone/logos/kubernetes/kubernetes-icon.svg" width="45" height="45" />
              <br />
              <sub>K8s</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://sonarqube.org/">
              <img
                src="https://assets-eu-01.kc-usercontent.com/ef593040-b591-0198-9506-ed88b30bc023/8e59bcad-6e39-41dc-abd9-a0e251e8d63f/Sonar%20%282%29.svg?w=128&h=32&dpr=2&fit=crop&q=80"
                width="45"
                height="45" />
              <br />
              <sub>SonarQube</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://jestjs.io">
              <img src="https://www.vectorlogo.zone/logos/jestjsio/jestjsio-icon.svg" width="45" height="45" />
              <br />
              <sub>Jest</sub>
            </a>
          </td>
        </tr>
      </table>
    </div>
    <div id="managemnt">
      <h3 align="center">🚀 Gestão, Design & Dados</h3>
      <table align="center">
        <tr>
          <td align="center">
            <a href="https://www.postgresql.org">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original.svg" width="45" height="45" />
              <br />
              <sub>Postgres</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.mongodb.com/">
              <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original.svg" width="45" height="45" />
              <br />
              <sub>MongoDB</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://cassandra.apache.org/">
              <img src="https://www.vectorlogo.zone/logos/apache_cassandra/apache_cassandra-icon.svg" width="45" height="45" />
              <br />
              <sub>Cassandra</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.palantir.com/platforms/foundry/">
              <img src="https://upload.wikimedia.org/wikipedia/commons/1/13/Palantir_Technologies_logo.svg" width="80" height="30" />
              <br />
              <sub>Palantir</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://clickup.com/">
              <img src="https://raw.githubusercontent.com/vinceliuice/Fluent-icon-theme/8a99a6dee4b2b8033bd3a41787792910d9d40c20/src/scalable/apps/clickup.svg" width="45" height="45" />
              <br />
              <sub>ClickUp</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.atlassian.com/software/jira">
              <img src="https://www.vectorlogo.zone/logos/atlassian_jira/atlassian_jira-icon.svg" width="45" height="45" />
              <br />
              <sub>Jira</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://www.figma.com/">
              <img src="https://www.vectorlogo.zone/logos/figma/figma-icon.svg" width="45" height="45" />
              <br />
              <sub>Figma</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://git-scm.com/">
              <img src="https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg" width="45" height="45" />
              <br />
              <sub>Git</sub>
            </a>
          </td>
          <td>&nbsp;&nbsp;</td>
          <td align="center">
            <a href="https://postman.com">
              <img src="https://www.vectorlogo.zone/logos/getpostman/getpostman-icon.svg" width="45" height="45" />
              <br />
              <sub>Postman</sub>
            </a>
          </td>
        </tr>
      </table>
    </div>
  </div>
  <br />
  <hr />
  <div id="stats">
    <h3 align="center">📊 Minhas Estatísticas GitHub</h3>
    <p align="center">
      <img src="https://github-readme-streak-stats.herokuapp.com/?user=rhayonne&theme=tokyonight" alt="rhayonne streak" />
    </p>
  </div>
</div>
